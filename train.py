...
def main():
    ...
    run: Run = wandb.init(project="truecaser")

    wandb.config.update(args)

    categories: Categories = BaseWordCategories()
    corpus: Corpus = get_corpus(args.corpus)
    embeddings: TokenEmbeddings = get_embeddings(args.embeddings, args.fine_tune)
    ...
    wandb.watch(model)
    trainer: TruecaserTrainer = TruecaserTrainer(model, corpus, optimizer=Adam)
