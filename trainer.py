...
class TruecaserTrainer(ModelTrainer):
    ...
    def train(
        ...
        if (isinstance(lr_scheduler, (STLR, OneCycleLR)) and batch_growth_annealing):
        ...
        try:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                log_line(log)
                wandb_metrics: dict = {"epoch": self.epoch}
                ...
                if (not isinstance(lr_scheduler, (STLR, OneCycleLR))) and learning_rate < min_learning_rate:
                    log_line(log)
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    if isinstance(lr_scheduler, STLR):
                        lr_scheduler.step()
                        # get new learning rate
                        for group in optimizer.param_groups:
                            learning_rate = group["lr"]
                    elif isinstance(lr_scheduler, OneCycleLR):
                        lr_scheduler.step()
                        # get new learning rate
                        for group in optimizer.param_groups:
                            learning_rate = group["lr"]
                ...
                if self.use_wandb:
                    wandb_metrics.update({'train_loss': train_loss, 'epoch': self.epoch})
                    # wandb.log({'train_loss': train_loss, 'epoch': self.epoch})
                ...
                if log_dev:
                    ...
                    if self.use_wandb:
                        wandb_metrics.update({'dev_loss': dev_loss, 'dev_score': dev_eval_result.main_score})
                        # wandb.log({'dev_loss': dev_loss, 'epoch': self.epoch})
                        metric: dict = flatten_dict(dev_eval_result.metrics, prefix="dev")
                        wandb_metrics.update(metric)
                        # metric['epoch'] = self.epoch
                        # wandb.log(metric)
                        # wandb.log({'dev_score': dev_eval_result.main_score, 'epoch': self.epoch})
                ...
                if log_test:
                    ...
                    if self.use_wandb:
                        wandb_metrics.update({'test_loss': test_loss, 'test_score': test_eval_result.main_score})
                        # wandb.log({'test_loss': test_loss, 'epoch': self.epoch})
                        metric: dict = flatten_dict(test_eval_result.metrics, prefix="test")
                        wandb_metrics.update(metric)
                        # metric['epoch'] = self.epoch
                        # wandb.log(metric)
                        # wandb.log({'test_score': test_eval_result.main_score, 'epoch': self.epoch})
                ...
                elif not isinstance(lr_scheduler, (STLR, OneCycleLR)):
    ...
    def final_test(
        ...
        if self.use_wandb:
            wandb.log(flatten_dict(test_results.metrics))
            wandb.summary.update(flatten_dict(test_results.metrics))
