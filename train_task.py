...
def get_corpus(corpus_name):
    if corpus_name == 'wikiner':
        return WIKINER_ENGLISH(base_path='data', tag_to_bioes=None, in_memory=True)
...
def main():
    """
    Base script for training tagger model
    """
    # Training settings
    ...
    # CRF or softmax
    parser.add_argument('--use_crf', type=lambda x:bool(distutils.util.strtobool(x)), default='False')
    # Dropout
    ...
    run: Run = wandb.init(project="truecaser", group="tasks")
